<template>
  <div class="box-card reveal reveal-loaded" :class="{ noStyle }">
    <!-- Ajouter des class reveal pour rendre visible et invisible -->
    <div class="card">
...
const handleIntersect = function (entries, observer) {
  //entries sont les élments que je vais observer
  entries.forEach(function (entry) {
    ...
    if (entry.intersectionRatio > threshold) {
      //Ici je compare l'intersectionRatio et le threshold que j'ai défini c'est > donc
      entry.target.classList.remove(
        "reveal"
      ); /* Je suprise la class reveal qui cache, pour faire apparaitre l'élément */
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  targets.forEach(function (target) {
    //Ici on dit que pour chaque élément récupérer execute cette fonction
...
.reveal-loaded .reveal {
  /* Ici on dit qu'avec la class reveal on veut tout rendre invisible */
  opacity: 0;
  transform: translateY(30px);
}

.reveal-loaded {
  /* Ici c'est le contraire avec cette class on veut tout faire réapparaitre */
