...
class Draw {
  ...
  static async getById(_id, width, height) {
    const { data: draw } = await API.get(`/draws/find`, {
      params: { _id, width, height },
    });
  ...
  static async groupBy(filter, width, height) {
    ...
    return Object.keys(drawsBy)
      .map((label) => ({ label, imgs: drawsBy[label] }))
      .sort((a, b) => a.label.localeCompare(b.label));
  }
  static async allByYear(width, height) {
    return Draw.groupBy(
      (draw) => dayjs(draw.date || "invalid").format("YYYY"),
      width,
      height
    );
  }
  static async allByMonth(width, height) {
    return Draw.groupBy(
      (draw) => dayjs(draw.date || "invalid").format("YYYY/MM"),
      width,
      height
    );
  }
  static async allByDay(width, height) {
    return Draw.groupBy(
      (draw) => dayjs(draw.date || "invalid").format("YYYY/MM/DD"),
      width,
      height
    );
  }
